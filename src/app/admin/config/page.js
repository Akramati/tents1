  ...
  return (
    <div className="container">
      ...
      <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.5rem" }}>
        <button className={`btn ${tab === "types" ? "btn-primary" : "btn-secondary"}`} onClick={() => setTab("types")} style={{ padding: "0.5rem 1.5rem", borderRadius: "8px", fontWeight: "bold", cursor: "pointer", border: "none" }}>📋 الأنواع والحقول</button>
        <button className={`btn ${tab === "message" ? "btn-primary" : "btn-secondary"}`} onClick={() => setTab("message")} style={{ padding: "0.5rem 1.5rem", borderRadius: "8px", fontWeight: "bold", cursor: "pointer", border: "none" }}>💬 قالب الرسالة</button>
        <button className={`btn ${tab === "field-accounts" ? "btn-primary" : "btn-secondary"}`} onClick={() => setTab("field-accounts")} style={{ padding: "0.5rem 1.5rem", borderRadius: "8px", fontWeight: "bold", cursor: "pointer", border: "none" }}>🚛 المصاريف الميدانية</button>
        <button className={`btn ${tab === "system" ? "btn-primary" : "btn-secondary"}`} onClick={() => setTab("system")} style={{ padding: "0.5rem 1.5rem", borderRadius: "8px", fontWeight: "bold", cursor: "pointer", border: "none" }}>⚙️ إعدادات النظام</button>
      </div>
      ...
      {tab === "types" && (
        <div className="config-layout">
          ...
          <section className="glass config-panel">
            <h2>📋 أنواع الحجوزات</h2>
            <div className="type-list">
              {types.map((t) => (
                <div key={t.typeName} className={`type-item ${selectedType === t.typeName ? "selected" : ""}`} onClick={() => { setSelectedType(t.typeName); setEditingField(null); setFieldForm({ fieldKey: "", fieldLabel: "", fieldType: "text", options: "", required: false }); }}>
                  <span className="type-icon">{t.icon || "📦"}</span>
                  <div className="type-info">
                    <strong>{t.typeName}</strong>
                    <span className={`behavior-badge behavior-${t.behavior}`}>{t.behavior === "packages" ? "باقات" : t.behavior === "hall" ? "صالة" : "مفردات"}</span>
                    {t.accountCode && <span className="behavior-badge" style={{background:"rgba(16,185,129,0.15)",color:"#10b981"}}>{t.accountCode}</span>}
                  </div>
                  <div className="type-actions">
                    <button className="btn btn-sm btn-ghost" onClick={(e) => { e.stopPropagation(); startEditType(t); }}>✏️</button>
                    <button className="btn btn-sm btn-ghost text-red" onClick={(e) => { e.stopPropagation(); handleDeleteType(t.typeName); }}>🗑️</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="add-form">
              <h3>{editingType ? "تعديل النوع" : "إضافة نوع جديد"}</h3>
          ...
          <section className="glass config-panel">
            ...
            {!selectedType ? (
              ...
              <>
                <h3 style={{ color: "var(--gold)" }}>حقول نوع: {types.find((t) => t.typeName === selectedType)?.icon} {selectedType}</h3>
                <div className="field-list">
                  {fieldsForSelected.length === 0 && <p className="no-data">لا توجد حقول مخصصة بعد</p>}
                  {fieldsForSelected.map((f) => (
                    <div key={f.fieldKey} className="field-item">
                      <div className="field-info">
                        <strong>{f.fieldLabel}</strong>
                        <span className="field-meta">{f.fieldKey} · {f.fieldType} {f.required ? "· إجباري" : ""} {f.isActive === false ? "· مخفي" : ""} {f.options?.length > 0 ? `· [${f.options.join(", ")}]` : ""}</span>
                      </div>
                ...
                <div className="add-form">
                  <h3>{editingField ? "تعديل الحقل" : "إضافة حقل جديد"}</h3>
                  ...
                  <label className="checkbox-label" style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginTop: "0.5rem" }}>
      ...
      {tab === "message" && (
        <section className="glass config-panel" style={{ maxWidth: "800px", margin: "0 auto" }}>
          <h2>💬 قوالب رسائل واتساب</h2>

          {/* Sub-tabs for template types */}
          <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1rem" }}>
            <button className={`btn ${msgType === "bookingConfirm" ? "btn-primary" : "btn-secondary"}`} onClick={() => { setMsgType("bookingConfirm"); loadMessage("bookingConfirm"); }} style={{ padding: "0.4rem 1rem", borderRadius: "8px", fontWeight: "bold", cursor: "pointer", border: "none", fontSize: "0.9rem" }}>
              📋 تأكيد الحجز
            </button>
            <button className={`btn ${msgType === "paymentReceipt" ? "btn-primary" : "btn-secondary"}`} onClick={() => { setMsgType("paymentReceipt"); loadMessage("paymentReceipt"); }} style={{ padding: "0.4rem 1rem", borderRadius: "8px", fontWeight: "bold", cursor: "pointer", border: "none", fontSize: "0.9rem" }}>
              🧾 سند قبض
            </button>
          </div>

          <p style={{ opacity: 0.8, marginBottom: "1rem" }}>{msgType === "bookingConfirm" ? "قم بتخصيص الرسالة التي ترسل للعميل عند تأكيد الحجز." : "قم بتخصيص الرسالة التي ترسل للعميل عند تسجيل دفعة."} استخدم المتغيرات بين `{ }` لإدراج البيانات.</p>
          ...
          <div style={{ marginBottom: "1rem", padding: "1rem", borderRadius: "12px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)" }}>
            <strong style={{ display: "block", marginBottom: "0.5rem" }}>🟢 المتغيرات المتاحة:</strong>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>
              {placeholders.map((p) => (
                <code key={p.key} style={{ background: "rgba(16,185,129,0.1)", padding: "0.25rem 0.5rem", borderRadius: "6px", fontSize: "0.85rem", cursor: "pointer", border: "1px solid rgba(16,185,129,0.2)" }}
      ...
      {tab === "field-accounts" && (
        <div className="config-layout" style={{ flexDirection: "column" }}>
          {["5101", "5102", "5103", "5104"].map((parentCode) => {
            ...
            return (
              <section key={parentCode} className="glass config-panel">
                <h2>{stageIcons[parentCode]} تكاليف {stageNames[parentCode]} (الرمز {parentCode})</h2>
                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", marginBottom: "1rem" }}>
                  {stageAccounts.map((a) => (
                    <div key={a.accountCode} style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.5rem 0.75rem", background: "rgba(255,255,255,0.05)", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.1)" }}>
                      <div>
                ...
                <div className="add-form">
                  <h3>{editingAcct && acctForm.parentCode === parentCode ? "تعديل حساب" : "إضافة حساب جديد"}</h3>
                      ...
                      <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.5rem" }}>
                        {editingAcct && acctForm.parentCode === parentCode ? (
                          <>
                            ...
                            <button className="btn btn-secondary" onClick={() => setEditingAcct(null)}>إلغاء</button>
                          </>
                        ) : (
                          <button className="btn btn-primary" onClick={async () => {
