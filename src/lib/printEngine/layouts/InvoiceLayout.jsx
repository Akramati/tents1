...
const InvoiceLayout = forwardRef(({ data, settings, previewSettings, formatDateArabic }, ref) => {
  ...
  if (beh === "hall") {
    ...
    const pkgInfo = data.packageUsed ? `🎁 ${data.packageUsed}` : "";
  ...
  return (
    <div ref={ref} className="ps-root" dir="rtl">
      ...
      <table className="ps-details"><tbody>
        <tr><td className="ps-label">رقم العقد / الحجز</td><td className="ps-value">{data.bookingId}</td></tr>
        <tr><td className="ps-label">اسم العميل</td><td className="ps-value">{data.customerName}</td></tr>
        <tr><td className="ps-label">رقم الجوال</td><td className="ps-value">{data.customerPhone}</td></tr>
        ...
        {data.tentCount && <tr><td className="ps-label">عدد الخيام</td><td className="ps-value">{data.tentCount}</td></tr>}
        ...
        {data.customerIdNumber && <tr><td className="ps-label">رقم البطاقة</td><td className="ps-value">{data.customerIdNumber}</td></tr>}
        {data.guarantorName && <tr><td className="ps-label">الضامن</td><td className="ps-value">{data.guarantorName}{data.guarantorPhone ? ` (${data.guarantorPhone})` : ""}</td></tr>}
      </tbody></table>
      <table className="ps-financial">
        <thead><tr><th>البيان</th><th>المبلغ</th></tr></thead>
        <tbody>
          <tr><td>إجمالي مبلغ الحجز</td><td className="ps-amount">{data.totalAmount?.toLocaleString?.() || data.totalAmount} ريال</td></tr>
          <tr><td>المبلغ المدفوع (مقدم)</td><td className="ps-amount">{data.paidAmount?.toLocaleString?.() || data.paidAmount} ريال</td></tr>
          <tr className="ps-remaining"><td>المبلغ المتبقي</td><td className="ps-amount">{data.remainingAmount?.toLocaleString?.() || data.remainingAmount} ريال</td></tr>
        </tbody>
      </table>
      {data.notes && <div className="ps-notes">ملاحظات: {data.notes}</div>}
      <table className="ps-signature"><tbody>
        <tr>
          <td className="ps-sign-col"><div className="ps-sign-line"></div><div>توقيع المشرف</div></td>
          <td className="ps-sign-col"><div className="ps-sign-line"></div><div>توقيع وموافقة العميل</div></td>
        </tr>
      </tbody></table>
    </div>
  );
